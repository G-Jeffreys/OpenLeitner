...
const INTERVALS: Record<number, number> = { 1: 0, 2: 1, 3: 3, 4: 7, 5: 21 } as const; // days

function applyLeitnerUpdate(prevBox: number, quality: 0|1|2|3) {
  const up = quality >= 2;
  const nextBox = up ? Math.min(prevBox + 1, MAX_BOX) : 1;
  const days = INTERVALS[nextBox as keyof typeof INTERVALS] ?? 0;
...
export async function registerStudyRoutes(app: FastifyInstance) {
  ...
  app.post('/v1/study/review', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const { nextBox, nextDueAt } = applyLeitnerUpdate(prevBox, quality as 0|1|2|3);
